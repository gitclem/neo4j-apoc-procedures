...
If the APOC procedure or function name contains `.do.` in the name, then query can be a **write** query.  Otherwise the query must be **read-only**.
...
CALL apoc.when(
  condition: Boolean,
  ifReadOnlyQuery: String,
  elseReadOnlyQuery: String,
  params: Map)
YIELD value
----
...
CALL apoc.when(
  ...
  'RETURN aNodes as resultNodes',  // Read Query
...
Or, if we wanted to use **write** queries to conditionally `SET` or `CREATE`/`MERGE` graph elements if we deem some account to be suspicious, but still want to continue other query operations in either case, we would use `apoc.do.when`:
...
CALL apoc.do.when(
  r IS NOT NULL,
  'SET acc:Suspicious',  // Write Query
  '',
  {acc:acc})
YIELD value
...
CALL apoc.case([
  $selection = 'friends', "RETURN [(me)-[:FRIENDS]-(friend) | friend] as selection",  // Read-Only Query
...
Or if we want to `CREATE`/`MERGE` (a write query) different relationship types between two nodes based on the value
...
CALL apoc.do.case([
  $selection = 'friends', "MERGE (me)-[rel:FRIENDS]->(friend) RETURN rel",    // Write Query
